   ...
   implements WorklistServer, Runnable, MessageBusListener, InfoSetupInterface
{
     ...
     throws Exception
  {
    ...
    for(InfInEsami pre : prestazioni)
    {
      ...
      StpCategorie cat = esa.getStpCategorie(con);
      ...
      String razza = "";
      if(ana.getIdRazza() != 0)
        razza = ana.getAnaRazze(con).getDescrizione();
     ...
     throws Exception
  {
    switch(msgID)
    {
      //case ArgoBusMessages.ACC_SAVED:
      //case ArgoBusMessages.ACC_REQUEUE:
      case ArgoBusMessages.GENERIC_OBJECT_SAVED:
        InfInAccettazioni acc = context.getParam(InfInAccettazioni.class);
        if(acc != null)
          PeerTransactAgent.executeReadonly((con) -> updateWorklist(acc, con));
        break;
    }
     ...
     throws Exception
  {
    ...
    String sSQL
       = "DELETE FROM RIS.WORKLIST"
       + " WHERE SCHED_PROC_STEP_START_DATE_TIME < ?";

    PeerTransactAgent.execute((con) ->
    {
      try (PreparedStatement ps = con.prepareStatement(sSQL))
      {
        ps.setDate(1, DbUtils.cvtDate(startDate));
        ps.executeUpdate();
      }
    });
  }
